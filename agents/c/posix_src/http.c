...
char* INITIAL_REQUEST_TEMPLATE = "%s %s HTTP/1.1\r\nConnection: Close\r\nUser-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.74 Safari/537.37 Edg/79.0.309.43\r\nPragma: no-cache\r\nHost: %s\r\nAccept: text/html\r\nAccept-Encoding: identity\r\n\r\n";
...
unsigned char* http_request(int isinitial, char* method, char* hostname, uint16_t port, char* cookie, char* path, char* postData, size_t postDataLen){
    ...
    if (isinitial == 1){
        data = calloc(strlen(INITIAL_REQUEST_TEMPLATE) + strlen(hostname) + strlen(path)+255+postDataLen,  1);
        sprintf(data, INITIAL_REQUEST_TEMPLATE, method, path, hostname);
    }
    else{
        data = calloc(strlen(REQUEST_TEMPLATE) + strlen(hostname) + strlen(path)+strlen(cookie)+255+postDataLen,  1);
    ...
    while (total_read_write_size < dataLen){
        read_write_size = send(sock, data+total_read_write_size, dataLen-total_read_write_size, 0);
    ...
    while (read_write_size > 0 && failcount < 2){
        read_write_size = recv(sock, outdata+total_read_write_size, CHUNKSIZE, 0);
...
int main(int argc, char* argv[]){
    ...
    outdata = http_request("GET", argv[1], (uint16_t)atoi(argv[2]), argv[3], NULL, 0);
    return 0;
}
#endif
